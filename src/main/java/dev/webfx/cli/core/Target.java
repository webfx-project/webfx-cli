...
public final class Target {
    ...
    public List<Platform> getSupportedPlatforms() {
       if (supportedPlatforms == null) {
            supportedPlatforms =
                ...
                .filter(this::isPlatformSupported)
                .toList();
        }
    ...
    public boolean isPlatformSupported(Platform platform) {
        ...
        return platformTags.isEmpty()
               || platformTags.stream().allMatch(tag -> tag.isPlatformSupported(platform));
    }
    ...
    public boolean isAnyPlatformSupported(Platform platform) {
        ...
        return platformTags.isEmpty()
               || platformTags.stream().anyMatch(tag -> tag.isPlatformSupported(platform));
    }
    ...
    int gradeTargetMatch(Target requestedTarget) {
        ...
        for (TargetTag requestedTag : requestedTarget.getTags()) {
            ...
            for (TargetTag tag : tags) {
                ...
                if (tagGrade < 0 && tags.size() > 1 && tags.stream().anyMatch(t -> t.isPlatformCompatible(requestedTag)))
